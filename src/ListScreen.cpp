...
void ListScreen::_displayItems() {
	...
	for (uint8_t i {0}; i < 2 && _definition.items[i].displayName[0]; ++i) {
		...
		if (item.icon) {
			_environment.canvas
			    .drawBitmap(21 + 54 * i, 7, reinterpret_cast<const uint8_t*>(item.icon), 32, 32, fgColor, bgColor);
		}
...
void ListScreen::onKeyUp(uint8_t key) {
	switch (key) {
		case 0:
		case 6: {
			_definition.items[_activeItem].callback(_activeItem);
			break;
		}
		case 1: {
			_definition.items[_leftItem].callback(_leftItem);
			break;
		}
		case 2: {
			_definition.items[_leftItem + 1].callback(_leftItem + 1);
			break;
		}
		case 3: {
			navigator.close();
